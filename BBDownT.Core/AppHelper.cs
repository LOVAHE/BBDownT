...
static class AppHelper
{
    ...
    public static byte[] ReadMessage(byte[] data)
    {
        if (data.Length < 5)
        {
            throw new InvalidDataException("gRPC响应报文长度不足");
        }

        byte first;
        int size;
        (first, size) = ReadInfo(data);
        if (size < 0 || size > Config.MAX_GRPC_MESSAGE_BYTES)
        {
            throw new InvalidDataException($"gRPC响应报文超过限制: {size} bytes");
        }

        if (data.Length < 5 + size)
        {
            throw new InvalidDataException("gRPC响应报文长度与头部声明不一致");
        }

        var payload = data[5..(5 + size)];
        return first == 1 ? GzipDecompress(payload, Config.MAX_GRPC_MESSAGE_BYTES) : payload;
    }
    ...
    private static byte[] GzipDecompress(byte[] data, int maxBytes)
    {
        ...
        using (var input = new MemoryStream(data))
        {
            ...
            int read;
            while ((read = decomp.Read(buffer, 0, buffer.Length)) > 0)
            {
                if (output.Length + read > maxBytes)
                {
                    throw new InvalidDataException($"gRPC gzip响应解压后超过限制: {maxBytes} bytes");
                }
